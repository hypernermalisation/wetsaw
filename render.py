...
# map extent options
parser.add_option("-n", "--north", dest="north", default=None, type="float",
                  help="Northern map extent default: [projection max]")
parser.add_option("-e", "--east", dest="east", default=None, type="float",
                  help="Eastern map extent default: [projection max]")
parser.add_option("-s", "--south", dest="south", default=None, type="float",
                  help="Southern map extent default: [projection max]")
parser.add_option("-w", "--west", dest="west", default=None, type="float",
                  help="Western map extent default: [projection max]")

parser.add_option("-p", "--projection", dest="projection", default="geo", type="string",
                  help="The projection to be used. options [geo, sm], default: geo")
...
parser.add_option("--watermark", dest="watermark", default=False, action="store_true",
                  help="Enable watermarking")
parser.add_option("--watermark_file", dest="watermark_file", default="./watermark.png", type="string",
                  help="The file to use for watermarking. Default is ./watermark.png")

# file handling options
parser.add_option("-k", "--keep-existing", dest="overwrite", default=True, action="store_false",
                  help="Keep files that already exist? default: not kept")
parser.add_option("-f", "--output-format", dest="output_format", default='png', type="string",
                  help="The output file format. Only 'png' and 'jpeg' are allowed default: png")
parser.add_option("-b", "--base-directory", dest="base_dir", default='./tiles/', type="string",
                  help="The base directory in which to store tiles default: ./tiles/")
parser.add_option('-g', '--log-file', dest='log_file', default='./render.log', type='string',
                  help="The file to append long information to, default: 'render.log' in the current directory")
parser.add_option('-i', '--pixel-bits', dest="pixel_bits", default=8, type='int',
                  help="The bits per pixel to use for .png renderings, allowed are '8', '24', and '32'. default: 8")
parser.add_option("-d", "--do-not-delete-metapanels", dest="preserve_metapanels", default=False, action="store_true",
                  help="Preserve meta panels after finished cutting tiles from them (default: no)")
parser.add_option("-W", "--website-mode", dest="website_mode", default=False, action="store_true",
                  help="Produce special output designed for control by/interaction wtih a website. default: no")
parser.add_option('-H', '--add-highlight', dest='highlight', default=None, type='string',
...
# s3 upload options
parser.add_option("--s3-file", dest="s3_file", default="/data/.s3",
...
parser.add_option("--s3-access", dest="s3_access", default=None,
                  help="S3 access key. Default is to gather it from the s3 file")
parser.add_option("--s3-secret", dest="s3_secret", default=None,
                  help="S3 secret key. Default is to gather it from the s3 file")
parser.add_option("--s3-bucket", dest="s3_bucket", default=None,
                  help="Bucket to use when uploading to s3. S3 uploads will not be performed unless this option is set")
parser.add_option("--s3-prefix", dest="s3_prefix", default=None,
...
parser.add_option("-P", "--generate-preview-images", dest="do_previews", default=False, action="store_true",
                  help="Generate a 316x316 preview along with the tiles, and resample a 61x61 thumbnail from that. default: do not generate")
parser.add_option("-K", "--generate-kml-preview", dest="do_kml", default=False, action="store_true",
...
parser.add_option("-Z", "--generate-tilepack", dest="generate_tilepack", default=None, type="string",
...
# zoom level options
parser.add_option("-c", "--scale", dest="scale", default=0, type="int",
                  help="The scale of the furthest zoomed out zoom level")
parser.add_option("-l", "--first-scale", dest="first_scale", default=1, type="int",
                  help="The first scale to start rendering at, default: the input to '-c'")
parser.add_option("-z", "--zoom-levels", dest="zoom_levels", default=5, type="int",
